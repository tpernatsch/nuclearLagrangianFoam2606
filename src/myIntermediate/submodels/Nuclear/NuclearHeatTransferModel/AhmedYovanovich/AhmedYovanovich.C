...
{
    // Aggressive check for NaN/Inf/unphysical values immediately upon entering the function
    // This check must be the very first operation to catch issues during argument passing
    if (Foam::isNan(Re) || Foam::isNan(Pr) || Foam::isInf(Re) || Foam::isInf(Pr) || Re <= 0 || Pr <= 0)
    {
        WarningInFunction
            << "AhmedYovanovich::Nu - Detected unphysical (NaN/Inf/<=0) Re (" << Re << ") or Pr (" << Pr << ") upon entry. "
            << "Returning default Nusselt number (2.0)."
            << endl;
        return 2.0;
    }
