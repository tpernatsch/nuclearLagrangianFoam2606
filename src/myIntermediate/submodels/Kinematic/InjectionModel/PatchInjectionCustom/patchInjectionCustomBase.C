...
#include "patchInjectionCustomBase.H"
#include "polyMesh.H"
#include "SubField.H"
#include "Random.H"
#include "triangle.H"
...
Foam::patchInjectionCustomBase::patchInjectionCustomBase
(
    const polyMesh& mesh,
...
Foam::patchInjectionCustomBase::patchInjectionCustomBase(const patchInjectionCustomBase& pib)
:
...
void Foam::patchInjectionCustomBase::updateMesh(const polyMesh& mesh)
{
...
Foam::label Foam::patchInjectionCustomBase::setPositionAndCell
(
    const fvMesh& mesh,
...
)
{
    ...
    if (cellOwners_.size() > 0)
    {
        ...
        if (Pstream::myProcNo() == proci)
        {
            ...
            // Existing code where y-coordinates would be adjusted
            scalar y = rnd.sample01<scalar>();                         // Generates random number between 0 and 1
            scalar adjustedY = (2*0.05/Foam::constant::mathematical::pi) * std::asin(2*y - 1);          // Apply reverse cumulative distribution function
            position.y() = adjustedY;                                  // Use this value for position
...
Foam::label Foam::patchInjectionCustomBase::whichProc(const scalar fraction01) const
{
    const scalar areaFraction = fraction01*patchArea_;
