...
#include "FunctionalRemoveParcels.H"
#include "fvMesh.H"
#include "faceZone.H"
#include "OFstream.H"
...
void Foam::FunctionalRemoveParcels<CloudType>::makeLogFile
(
    const word& zoneName,
    const label zoneI,
    const label nFaces,
...
template<class CloudType>
void Foam::FunctionalRemoveParcels<CloudType>::postEvolve
(
...
template<class CloudType>
void Foam::FunctionalRemoveParcels<CloudType>::write()
{
    const fvMesh& mesh = this->owner().mesh();
...
template<class CloudType>
Foam::FunctionalRemoveParcels<CloudType>::FunctionalRemoveParcels
(
...
Foam::FunctionalRemoveParcels<CloudType>::FunctionalRemoveParcels
(
    const FunctionalRemoveParcels<CloudType>& rpf
)
:
    ...
    mass_(rpf.mass_),
    typeId_(rpf.typeId_),
    probabilityPtr_(rpf.probabilityPtr_.clone()),
...
template<class CloudType>
bool Foam::FunctionalRemoveParcels<CloudType>::postFace
(
    const parcelType& p,
    const typename parcelType::trackingData& td
)
{
    ...
    )
    {
        ...
        forAll(faceZoneIDs_, i)
        {
            ...
            if (fz.found(p.face()))
            {
                // Removal probability for this parcel's diameter
                const scalar Premove = probabilityPtr_->value(p.d());

                if (this->owner().rndGen().template sample01<scalar>() < Premove)
                {
                    nParcels_[i]++;
