...
void Foam::KinematicCloud<CloudType>::info()
{
    ...
    const scalar totDecayedParticles =
        returnReduce(totalDecayedParticles(), sumOp<scalar>());

    const scalar totDecayedParcels =
        returnReduce(totalDecayedParcels(), sumOp<scalar>());
    ...
    if (constProps_.lambda() != 0)
    {
        Log_<< "    Number of decayed particles     = " << totDecayedParticles << nl
            << "    Number of decayed parcels     = " << totDecayedParcels << nl;
    }
    if (constProps_.tau() > 0)
    {
        const scalar maxParcelDiameter = returnReduce(this->Dmax(), maxOp<scalar>());

        Log_<< "    Maximum parcel diameter     = " << maxParcelDiameter << nl;
    }
