...
void Foam::KinematicParcel<ParcelType>::readFields(CloudType& c)
{
    ...
    IOField<scalar> nParticlesDecayed
    (
        c.fieldIOobject("nParticlesDecayed", IOobject::MUST_READ),
        readOnProc
    );
    c.checkFieldIOobject(c, nParticlesDecayed);

    IOField<scalar> nParcelsDecayed
    (
        c.fieldIOobject("nParcelsDecayed", IOobject::MUST_READ),
        readOnProc
    );
    c.checkFieldIOobject(c, nParcelsDecayed);
    ...
    for (KinematicParcel<ParcelType>& p : c)
    {
        ...
        p.nParticlesDecayed_ = nParticlesDecayed[i];
        p.nParcelsDecayed_ = nParcelsDecayed[i];
        p.d_ = d[i];
...
void Foam::KinematicParcel<ParcelType>::writeFields(const CloudType& c)
{
    ...
    IOField<scalar> nParticlesDecayed
    (
        c.fieldIOobject("nParticlesDecayed", IOobject::NO_READ),
        np
    );
    IOField<scalar> nParcelsDecayed
    (
        c.fieldIOobject("nParcelsDecayed", IOobject::NO_READ),
        np
    );
    ...
    for (const KinematicParcel<ParcelType>& p : c)
    {
        ...
        nParticlesDecayed[i] = p.nParticlesDecayed();
        nParcelsDecayed[i] = p.nParcelsDecayed();
        d[i] = p.d();
        dTarget[i] = p.dTarget();
        U[i] = p.U();
        rho[i] = p.rho();
    ...
    nParticlesDecayed.write(writeOnProc);
    nParcelsDecayed.write(writeOnProc);
    d.write(writeOnProc);
...
) const
{
    ...
    writeProp("nParticlesDecayed", nParticlesDecayed_);
    writeProp("nParcelsDecayed", nParcelsDecayed_);
    writeProp("d", d_);
...
)
{
    ...
    const auto& nParticlesDecayed = cloud::lookupIOField<scalar>("nParticlesDecayed", obr);
    const auto& nParcelsDecayed = cloud::lookupIOField<scalar>("nParcelsDecayed", obr);
...
)
{
    ...
    auto& nParticlesDecayed = cloud::createIOField<scalar>("nParticlesDecayed", np, obr);
    auto& nParcelsDecayed = cloud::createIOField<scalar>("nParcelsDecayed", np, obr);
...
)
{
    if (os.format() == IOstreamOption::ASCII)
    {
        os  << static_cast<const ParcelType&>(p)
            ...
            << token::SPACE << p.nParticlesDecayed()
            << token::SPACE << p.nParcelsDecayed()
            << token::SPACE << p.d()
