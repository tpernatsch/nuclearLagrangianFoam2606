...
)
{
    ...
    this->U_ =
        calcVelocity(cloud, td, dt, Re, td.muc(), mass0, Su, dUTrans, Spu); //add kappa for the diffusion implementation
...
) const
{
    ...
    //Pseudo-random motion of smaller particles due to diffusion
    //First the shifting, then the computation of velocity

    //Pseudo-Diffusion coefficient calculation
    //D = kappa*cloud.constProps().D0()*p.d()**beta

    //while (p inside the domain){    ---> checks if the random movement didn't move the particle outside the domain
    //  p.positions += sqrt(D*dt)*normDistr(0,1)
    //}
...
:
    ...
    nParticlesDecayed_(p.nParticlesDecayed_),
    nParcelsDecayed_(p.nParcelsDecayed_),
    d_(p.d_),
    dTarget_(p.dTarget_),
    U_(p.U_),
    rho_(p.rho_),
...
)
{
    ...
    while (ttd.keepParticle && !ttd.switchProcessor && p.stepFraction() < 1)
    {
        ...
        const scalar nPartOld = p.nParticle();
