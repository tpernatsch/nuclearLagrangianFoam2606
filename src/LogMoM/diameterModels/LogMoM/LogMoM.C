...
{
    return
        phase().mesh().lookupObject<phaseCompressibleTurbulenceModel>
        (
            IOobject::groupName
            (
...
{
    return orderedPhasePair(phase(), continuousPhase());
}
...
void Foam::diameterModels::LogMoM::correct()
{
    ...
    Foam::fv::options& fvOptions(Foam::fv::options::New(phase().mesh()));
